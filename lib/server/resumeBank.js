...
BULLET POOL SELECTION — CHOOSE BEFORE YOU REWRITE:
- Each entry's "original_bullets" is a POOL of everything the user has written for that experience — it may hold many more bullets than belong on a one-page resume. Do not try to use all of them.
- For every entry (MUST-INCLUDE or selected CANDIDATE), first choose the 2-3 bullets from its pool that are most relevant to THIS JD. If the pool has 3 or fewer bullets, you may use all of them. Only rewrite the bullets you chose — leave the rest out of the resume entirely.
- The same entry should surface different bullets for different JDs: for a data/analytics JD, choose the bullets emphasizing technical methods, modeling, or tools; for a marketing/growth JD, choose the ones emphasizing growth, channels, or strategy — pick whichever REAL pool bullets best match this specific JD.
- Every rewritten bullet you output must declare which pool bullet it's based on via "source_index" (see output shape below) — you may only rewrite bullets you actually selected from the real pool; you cannot output a bullet with no corresponding pool entry.

BULLET REWRITING — BE BOLD IN FRAMING, NEVER IN FACTS:
- Open every bullet with a strong, specific action verb (e.g. Engineered, Led, Drove, Built, Designed, Automated, Analyzed) — never a passive or weak opener ("Responsible for", "Helped with", "Worked on").
- Actually restructure the sentence — don't just swap in a synonym or two. Rebuild it so the skill, method, or quantified result most relevant to THIS JD leads the bullet, with supporting context after.
- Foreground whichever real skills, methods, and results in the bullet you selected are most relevant to this JD — you are choosing which true facts to foreground and how to phrase them, never changing what happened.
- Any quantified result in the original bullet must stay in the rewrite, ideally moved earlier in the sentence — never bury a real number, and never add one that isn't in the original.
...
ABSOLUTE RED LINES — you are rewriting HOW something is described, never WHAT happened:
...
- Rewriting means rephrasing the user's REAL selected bullet — never adding a fact, metric, or achievement that isn't a reworded version of the pool bullet at "source_index". You may leave pool bullets unselected, but never invent a new one with no source.
- Never invent, exaggerate, or round up a number, percentage, or scale (team size, budget, users, growth %, timeframe) beyond what the original bullet states.
- Never upgrade the level of responsibility or seniority implied by the original wording — e.g. "participated in", "supported", "assisted with" must never become "led", "owned", "spearheaded", "drove"; those verbs are only allowed if the original bullet already describes that level of ownership.
- "Bolder" means stronger verbs and sentence structure, never bolder (i.e. inflated) facts. Every number, responsibility, and outcome in your rewrite must be traceable to the original data you were given.
...

{
  ...
  "bullets": {
    "<experience_id>": [ { "source_index": <integer — 0-based index into that entry's original_bullets pool that this rewrite is based on>, "text": "<rewritten bullet>", "bold": [<0-2 exact substrings of "text" to bold>] } ]
  },
...
"bullets" must have an entry for every MUST-INCLUDE id and every id you put in "selected_experience_ids" — 2-3 selected+rewritten bullets each (fewer only if that entry's real pool has fewer than 3 bullets total). Return nothing but the JSON object.`;
...
- Open every bullet with a strong, specific action verb (e.g. Engineered, Led, Drove, Built, Designed, Automated, Analyzed) — never a passive or weak opener ("Responsible for", "Helped with", "Worked on").
- Actually restructure the sentence — don't just swap in a synonym or two. Rebuild it so the skill, method, or quantified result most relevant to THIS JD leads the bullet, with supporting context after.
- Foreground whichever real skills, methods, and results in the bullet are most relevant to this JD — you are choosing which true facts to foreground and how to phrase them, never changing what happened.
- Any quantified result in the original bullet must stay in the rewrite, ideally moved earlier in the sentence — never bury a real number, and never add one that isn't in the original.
...
- Never invent, exaggerate, or round up a number, percentage, or scale beyond what the original states.
- Never upgrade the level of responsibility or seniority implied by the original wording — e.g. "participated in", "supported", "assisted with" must never become "led", "owned", "spearheaded", "drove"; those verbs are only allowed if the original already describes that level of ownership.
- "Bolder" means stronger verbs and sentence structure, never bolder (i.e. inflated) facts.
...
export function reconcileBankResponse({ experiences, mustInclude, parsed, profile }) {
  ...
  // A rewritten bullet is only trusted if it declares a "source_index" that
  // points at a real bullet in that entry's own pool — this is the code-level
  // check backing the "select from the real pool, never invent" prompt rule.
  // Any bullet without a valid source_index is dropped rather than trusted.
  const resolveBullets = (e) => {
    const poolSize = (e.bullets || []).length;
    const original = (e.bullets || [])
      .map((b) => ({ text: b.text, bold: [] }))
      .filter((b) => b.text);
    const modelBullets = parsed.bullets?.[e.id];
    if (Array.isArray(modelBullets)) {
      const grounded = modelBullets
        .filter(
          (b) =>
            b &&
            typeof b.text === 'string' &&
            b.text.trim() &&
            Number.isInteger(b.source_index) &&
            b.source_index >= 0 &&
            b.source_index < poolSize
        )
        .map((b) => ({
          text: b.text,
